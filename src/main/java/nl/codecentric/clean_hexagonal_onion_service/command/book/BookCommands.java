package nl.codecentric.clean_hexagonal_onion_service.command.book;

import nl.codecentric.clean_hexagonal_onion_service.domain.book.Book;
import nl.codecentric.clean_hexagonal_onion_service.domain.book.BookService;
import nl.codecentric.clean_hexagonal_onion_service.domain.publisher.PublisherService;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

/**
 * @author Maik Kingma
 */

@RestController
@RequestMapping("books/{id}/commands")
public class BookCommands {

    private final PublisherService publisherService;

    private final BookService bookService;

    public BookCommands(PublisherService publisherService, BookService bookService) {
        this.publisherService = publisherService;
        this.bookService = bookService;
    }

    @PostMapping("/publish")
    @ResponseStatus(code = HttpStatus.ACCEPTED)
    public void publishBook(@PathVariable("id") Long bookId, @RequestBody PublishBookDTO publishBookDTO) {
        var publisher = publisherService.getPublisherById(publishBookDTO.publisherId());
        Book book = bookService.findById(bookId);

        if (!book.canBePublished()) {
            throw new BookService.BookAlreadyInPublishingException("Book already in publishing!");
        }
        publisher.publishBook(book);

        bookService.storeManuscript(book);
    }
}
